...
- `ShiftActionsProps.shiftUp`/`shiftDown` are typed `(event?: MouseEvent) => void`. React types them `() => void` but wires them straight to `onClick` and then reads `event.altKey`, so the upstream type is wrong.
