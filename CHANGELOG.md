...
- The conformance extractor records each element's _own_ direct text-node children verbatim, matching the `text` channel added by upstream fixture `schemaVersion` 3. `CONFORMANCE_TAG` is now `v8.23.0`, the first release publishing schema 3, so the channel is asserted. Schema 2 fixtures are still accepted by the fetch script, with the channel dropped before comparison.
