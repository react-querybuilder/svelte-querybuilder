...
/**
 * The unit suite, on the bare `svelte()` plugin rather than `sveltekit()`. Tests import the
 * library by relative path and need nothing from Kit, and the Kit plugin resolves its project
 * from `process.cwd()` — which is the monorepo root when the root config runs this package as a
 * `projects` entry, so it would look for `src/app.html` in the wrong place.
 */
