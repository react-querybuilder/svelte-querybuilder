 ...
 * are self-contained—they carry no dependency on the downloaded fixture files—so this suite runs
 * under the default `bun run test` config.
 */
  ...
  for (const scenario of scenarios) {
    ...
    it(`has no axe violations: ${scenario.name}`, async () => {
      const { container } = render(QueryBuilder, {
        ...
        // `QueryBuilder`'s inference of `RG` from the query.
        props: { ...scenario.props, defaultQuery: query } as ComponentProps<typeof QueryBuilder>,
      });
...
describe('keyboard navigation', () => {
  it('reaches every control in a rule row in document order', async () => {
    ...
    // Start just before the rule: tabbing forward from there must walk the row in order.
  ...
  it('activates a button control with the keyboard', async () => {
    render(QueryBuilder, { props: { fields, defaultQuery: queries.singleRule } });
  ...
  it('associates the not-toggle label with its checkbox', async () => {
    render(QueryBuilder, {
      props: { fields, defaultQuery: queries.singleRule, showNotToggle: true },
    });
